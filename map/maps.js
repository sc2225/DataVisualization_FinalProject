...
function loadMap(settings) {
  ...
  d3.json("json/" + settings['path'] + ".json", function(error, layer) {
...
function drawChicago(hide) {
  ...
  d3.json("/json/chicago.json", function(error, layer) {
    if (error) return console.error(error);
...
$(document).ready(function() {
  ...
  var map = maps['neighborhoods'];
  var over = overlay['streets'];
